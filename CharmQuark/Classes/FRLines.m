...
void __ccpCurveDrawTexture(ccpCurve *curve){
	...
	if (curve->dirty) {
		...
		//	2px radius.
		float radius = (curve->width)/2.f+2.f;
		...
		CGPoint start = ccpMult(curve->vertices[0],1);
		CGPoint end = ccpMult(curve->vertices[1], 1);
		...
		for (int i = 0; i < curve->verticesCount; i++) {
			...
			start = ccpMult(curve->vertices[i], 1);
            
			if(0 <= i-1 && i+1 < curve->verticesCount) {
				CGPoint a = ccpMult(curve->vertices[i-1], 1);
				CGPoint b = ccpMult(curve->vertices[i+1], 1);
				dir = ccpNormalize(ccpAdd(ccpSub(start, a),ccpSub(b, start)));
			} else if (i+1 < curve->verticesCount) {
				end = ccpMult(curve->vertices[i + 1], 1);
		...
		start = ccpMult(curve->vertices[curve->verticesCount-2], 1);
		end = ccpMult(curve->vertices[curve->verticesCount-1], 1);
		dir = ccpNormalize(ccpSub(end, start));
		n = ccpPerp(dir);
    ...
    //// Changed to shader based draw.
    shader_ = [[CCShaderCache sharedShaderCache] programForKey:kCCShader_PositionTextureColor];
    [shader_ use];
    [shader_ setUniformForModelViewProjectionMatrix];	
    
    ccGLBlendFunc(CC_BLEND_SRC, CC_BLEND_DST);
    //ccGLBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
    ccGLBindTexture2D( [curve->texture name] );
    ccGLEnableVertexAttribs( kCCVertexAttribFlag_PosColorTex );
    
    glVertexAttribPointer(kCCVertexAttrib_Position, 3, GL_FLOAT, GL_FALSE, sizeof(ccV3F_C4B_T2F), (void*) (&curve->vertexGL[0].vertices));    
    
    glVertexAttribPointer(kCCVertexAttrib_TexCoords, 2, GL_FLOAT, GL_FALSE, sizeof(ccV3F_C4B_T2F), (void*)((&curve->vertexGL[0].texCoords )));
    
    glVertexAttribPointer(kCCVertexAttrib_Color, 4, GL_UNSIGNED_BYTE, GL_TRUE, sizeof(ccV3F_C4B_T2F), (void*)(&curve->vertexGL[0].colors ));
